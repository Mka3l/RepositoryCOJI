...
const FacturesImpayees = () => {
  ...
  const visibleColumns = [columns[1], ...columns.slice(columnRanges[currentIndex].start, columnRanges[currentIndex].end + 1)];
  
  const [rowsToShow, setRowsToShow] = useState(
    rows.map(row =>
      [row[1], ...row.slice(columnRanges[currentIndex].start, columnRanges[currentIndex].end + 1)]
    )
  );
  ...
  const showNextColumns = () => {
    ...
    if (nextIndex < columnRanges.length) {
      ...
      const newRowsToShow = rows.map(row =>
        [row[1], ...row.slice(columnRanges[nextIndex].start, columnRanges[nextIndex].end + 1)]
      );
  ...
  const showPreviousColumns = () => {
    ...
    if (previousIndex >= 0) {
      ...
      setCheckboxStates(rows.map(row => row.map(() => false))); // Réinitialiser les états des cases à cocher
      const newRowsToShow = rows.map(row =>
        [row[1], ...row.slice(columnRanges[previousIndex].start, columnRanges[previousIndex].end + 1)]
      );
      setRowsToShow(newRowsToShow);
    }
  };
