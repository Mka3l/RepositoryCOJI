...
const PlanNonCompetition = (delegation) => {
  ...
  // dddd
  const[nom_pays, setNom_pays] = useState('');
  const[discipline_id, setDiscipline_id] = useState('');

  const urltext = 'https://api-jioi.digital.gov.mg/api/v1/detail-restauration/';
  useEffect(() => {
    fetch(urltext, {
      method: 'GET',
  ...
  const voirListDetails =(data1,data2)=> {
    console.log((data1))
    console.log((data2))
  }; 
                ...
                if (row && row.Délégation === delegation.delegation) {
                  return (
                    <tr key={rowIndex}>
                      ...
                      <td>
                      <AjoutHebergement Action={"Voir Liste"}  Titre={"Liste Personnes de délégation "+row.Délégation +' - '+ row.Disciplines } OnClick={voirListDetails(row.Délégation,row.Disciplines)}>
                ...
                <tr key={rowIndex}>
                  ...
                  <td>
                    <AjoutHebergement Action={"Voir Liste"} Titre={"Liste Personnes de délégation "+row.Délégation +' - '+ row.Disciplines}>
                      <div style={styleListPersone} >
                        <h5>Liste Personnes de délégation {row.Délégation} - {row.Disciplines} </h5>
                        <table style={{... styleTable,border:"1px solid" }}>
                            <tr style={{... styleTable,border:"1px solid" }} >
                              <th style={{... styleTable,border:"1px solid" }}>Contact</th>
                              <th style={{... styleTable,border:"1px solid" }} >genre</th>
                              <th style={{... styleTable,border:"1px solid" }} >Nom</th>
                              <th style={{... styleTable,border:"1px solid" }} >Prenom</th>
                              <th style={{... styleTable,border:"1px solid" }} >Discipline</th>
                              <th style={{... styleTable,border:"1px solid" }} >Function</th>
                              <th style={{... styleTable,border:"1px solid" }} >Nom Pays</th>
                            </tr>
                            {  dataPerson.map((person,index) => (
                                <tr key={index} style={{...styleRow, textAlign:"center"}} border={"1px solid"}>
                                  <td border={"1px solid"} >{ person.Contact }</td>
